...
namespace project_euler.Problems.ConcreteProblems
{
    internal class Problem027 : BaseProblem, IProblem
    {
        ...
        private static int FindBestQuadraticPrimes()
        {
            ...
            var bCandidates = ListOfPrimes.GetPrimesBelow(1000).Reverse();
            ListOfPrimes.BuildUpTo(2000000); //n<b<1000 so n^2<1000000 and an<1000000 and b<10000
            foreach (var b in bCandidates)
            {
                for (int a = -999; a < 1000; a+=2)
                {
                    for (int n = 0; n < b; n++)
                    {
                        if (!PrimesCalculator.IsPrime((n * n) + (a * n) + b))
                        {
                            break;
                        }
